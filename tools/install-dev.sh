#!/usr/bin/env bash
# Build XivDesktop (Release) and print the path to add under
...
# it stages the build in ../xiv-desktop-build/devplugin (XIVDESKTOP_STAGE overrides).
#
# Environment:
#   DOTNET            dotnet executable (default: ~/.dotnet/dotnet, then dotnet on PATH)
#   XIVDESKTOP_ARTIFACTS  build output root (default: ../xiv-desktop-build/artifacts next to the repo)
set -euo pipefail

repo="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
project="$repo/src/XivDesktop.Plugin/XivDesktop.Plugin.csproj"
...
artifacts="${XIVDESKTOP_ARTIFACTS:-$(cd "$repo/.." && pwd -P)/xiv-desktop-build/artifacts}"
export DOTNET_CLI_TELEMETRY_OPTOUT=1 DOTNET_NOLOGO=1 XIVDESKTOP_ARTIFACTS="$artifacts"

echo "install-dev: building $project (Release) into $artifacts" >&2
"$dotnet" build "$project" -c Release -nologo -v quiet >&2

dll="$artifacts/bin/XivDesktop.Plugin/release/XivDesktop.dll"
if [[ ! -f "$dll" ]]; then
...
dll="$(readlink -f "$stage/XivDesktop.dll")"
...
Installed Dev Plugins, enable "XivDesktop" and tick "Start on boot".
After rebuilding, reload XivDesktop from /xlplugins (or tick its "Automatic reloading").
EOF
printf '%s\n' "$windows_path"
