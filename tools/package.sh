#!/usr/bin/env bash
# Package a built XivDesktop plugin for Dalamud, and write the one-entry plugin
...
#
#   dotnet build XivDesktop.slnx -c Release && tools/package.sh
#
# Output, all under $OUT (default ../xiv-desktop-build/release):
#   latest.zip                  the plugin folder as Dalamud installs it (XivDesktop.json,
#                               XivDesktop.dll, XivDesktop.Core.dll, no .pdb). The name is stable on purpose, so
#                               .../releases/latest/download/latest.zip never changes.
#   XivDesktop-<version>.zip       the same bytes under a self-describing name
...
#                      $XIVDESKTOP_ARTIFACTS/bin/XivDesktop.Plugin/release, else
#                      ../xiv-desktop-build/artifacts/bin/XivDesktop.Plugin/release)
#   OUT                where to write (default ../xiv-desktop-build/release)
#   REPO               owner/name on GitHub (default $GITHUB_REPOSITORY, else
#                      Spaceghost/xivdesktop-dalamud)
...
ARTIFACTS="${XIVDESKTOP_ARTIFACTS:-$ROOT/../xiv-desktop-build/artifacts}"
BIN="${BIN:-$ARTIFACTS/bin/XivDesktop.Plugin/release}"
OUT="${OUT:-$ROOT/../xiv-desktop-build/release}"
REPO="${REPO:-${GITHUB_REPOSITORY:-Spaceghost/xivdesktop-dalamud}}"
MANIFEST="$ROOT/src/XivDesktop.Plugin/XivDesktop.json"

[[ -f "$BIN/XivDesktop.dll" ]] || {
  echo "error: $BIN/XivDesktop.dll missing; run: dotnet build XivDesktop.slnx -c Release" >&2
  exit 1
}
...
STAGE="${TMPDIR:-/tmp}/xivdesktop-package-$$"
trap 'rm -rf "$STAGE"' EXIT
rm -rf "$STAGE"
mkdir -p "$STAGE/XivDesktop" "$OUT"
...
( cd "$BIN" && find . -type f -print0 | while IFS= read -r -d '' f; do
    ...
    case "$f" in
      ...
      ./runtimes/* | ./XivDesktop/*) continue ;;                          # other platforms; DalamudPackager's own staging copy
      ./*.deps.json | ./*.runtimeconfig.json | *.pdb | *.zip) continue ;;
      ./*/*) continue ;;                                               # nothing else from a subfolder
      ./*.dll | ./*.json) ;;
      *) continue ;;
    esac
    mkdir -p "$STAGE/XivDesktop/$(dirname "$f")"
    cp "$f" "$STAGE/XivDesktop/$f"
  done )
cp "$MANIFEST" "$STAGE/XivDesktop/XivDesktop.json"

# Dalamud unpacks the zip straight into the plugin folder, so the files sit at the root.
find "$STAGE/XivDesktop" -exec touch -h -d "@$SOURCE_DATE_EPOCH" {} +
rm -f "$OUT/latest.zip" "$OUT/XivDesktop-$VERSION.zip"
( cd "$STAGE/XivDesktop" && find . -type f | LC_ALL=C sort | sed 's|^\./||' |
    TZ=UTC zip -X -D -q "$OUT/latest.zip" -@ )
cp "$OUT/latest.zip" "$OUT/XivDesktop-$VERSION.zip"
echo "== $OUT/latest.zip"
...
for want in XivDesktop.dll XivDesktop.json ; do
