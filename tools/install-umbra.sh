#!/usr/bin/env bash
# Build the Umbra.XivDesktop custom Umbra plugin (the "Apps" toolbar launcher for XivDesktop).
#
#   tools/install-umbra.sh              build Release, print the DLL path and the Z:\ path for Umbra
#   tools/install-umbra.sh --install    also copy Umbra.XivDesktop.dll (+ .pdb) into $UMBRA_PLUGIN_DIR
#
# Environment:
#   DOTNET            dotnet binary              (default: ~/.dotnet/dotnet, then dotnet on PATH)
#   XIVDESKTOP_ARTIFACTS  bin/obj root for this build (default: ~/xiv-desktop-build/artifacts via Directory.Build.props)
...
# plugin in Umbra Settings -> Plugins and restarting Umbra are manual steps (see README.md, "Umbra widget").
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PROJECT="$ROOT/src/XivDesktop.Umbra/XivDesktop.Umbra.csproj"
INSTALL=0
...
export XIVDESKTOP_ARTIFACTS="$ARTIFACTS"

echo "== building Umbra.XivDesktop (Release)"
"$DOTNET" build "$PROJECT" -c Release -nologo "${props[@]}"

OUT="$(cd "$ARTIFACTS" && pwd)/bin/XivDesktop.Umbra/release"
DLL="$OUT/Umbra.XivDesktop.dll"
...
mkdir -p "$DEST"
cp -f "$DLL" "$DEST/Umbra.XivDesktop.dll"
if [[ -f "$OUT/Umbra.XivDesktop.pdb" ]]; then cp -f "$OUT/Umbra.XivDesktop.pdb" "$DEST/Umbra.XivDesktop.pdb"; fi
echo "installed: $DEST/Umbra.XivDesktop.dll"
echo "Add this path in Umbra Settings -> Plugins (once), then restart Umbra:"
echo "  $(winpath "$DEST/Umbra.XivDesktop.dll")"
